...
const LTC_MIN_FEE = new Decimal('0.00001');

export async function executeLtcVendorDisbursement(params: {
  tx: any;
  recv: any;
  ...
  const onChain = new Decimal(await getUtxoAddressBalance('litecoin', depositAddress));
  if (onChain.lessThan(recvAmount)) {
    throw ApiError.badRequest(
      `Insufficient on-chain LTC. Available: ${onChain.toString()}, required: ${recvAmount.toString()}`
    );
  }

  const feeLtc = await estimateUtxoTxFee('litecoin');
  const totalFee = Decimal.max(feeLtc, LTC_MIN_FEE);
  const netToVendor = recvAmount.minus(totalFee);

  if (!netToVendor.isFinite() || netToVendor.lte(0)) {
    throw ApiError.badRequest(
      `Receive amount too small to cover Litecoin network fee (${totalFee.toString()} LTC).`
    );
  }
  ...
  try {
    ...
    cryptoLogger.exception('LTC vendor disbursement failed', e, { receiveTransactionId });
  ...
  try {
    ...
    cryptoLogger.exception('DB update after LTC disbursement', e, { txHash, pendingId: pending.id });
